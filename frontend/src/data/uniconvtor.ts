...
export function asset(path: string) {
  // Empty or default placeholder path
  if (!path || path === "/products/default.jpg") {
    return `/uniconvtor/static/upload/image/20240717/1721203071201008.png`;
  }

  // Absolute URLs (Cloudinary, external CDN, etc.) – pass through directly
  if (path.startsWith("http://") || path.startsWith("https://")) {
    return path;
  }

  // Paths like /images/products/xxx.jpg were generated by the Admin form but
  // those files are not stored locally – fallback to a safe placeholder
